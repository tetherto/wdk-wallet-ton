...
describe('WalletAccountTon', () => {
  ...
  describe('sendTransaction', () => {
    ...
    test('should send a base64-looking string without the BoC magic prefix as a text comment', async () => {
      const COMMENT = 'dGVzdCBjb21tZW50'
    ...
    test('should throw on a broken BoC body instead of sending it as a text comment', async () => {
      const TRANSACTION = {
        to: RECIPIENT.address,
        value: 1_000_000,
        body: 'te6cckEBAQEAAgAAAA'
      }
  ...
  describe('transfer', () => {
    test('should successfully transfer tokens', async () => {
      ...
      mockCryptoRandomValues(expectedQueryId)
    ...
    test('should generate different hashes for identical token transfers (queryId ensures uniqueness)', async () => {
      ...
      mockCryptoRandomValues(
        calculateQueryId(0.1, 0.2),
        calculateQueryId(0.3, 0.4),
        calculateQueryId(0.5, 0.6)
      )
      global.Date.now = jest.fn(() => 3_000_000_000_000)
